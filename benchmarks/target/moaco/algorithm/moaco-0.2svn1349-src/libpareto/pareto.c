...
void ParetoUpdateWithSol(t_Pareto *p, t_Pareto_elem sol)
{
    ...
    while (j < p->size && !dominated) {
        /* If exist one solution in the pareto equal to this solution,
           ...
           nondominated set, therefore there is no solution in the
           nondominated set that can dominate this one, and there is
           no solution in the nondominated set dominated by this one.
           And, if there is one solution in the nondominated set that
           dominates this solution, then this solution can't donimate
           any solution in the nondominated set.
        */
        if (SolDominates(p->solutions[j], sol) >= 0) {
            dominated = TRUE;
        }
        /* If this solution dominates one solution in the nondominated set, then
	   there is not any solution in the nondominated set that dominates this
	   one.
        */
            ...
            /* We don't need to do anything more than check for other
               dominated solutions in the Nondominated Set */
            while(j < p->size) {
            ...
            /* Now, we add this solution to the nondominated set, so it becomes
               dominated */
            ParetoAddTo(p, sol);
            dominated = TRUE;
        }
        /* If nothing above, let's look next solution in the Nondominated Set */
        else {   j++;    }
    }
