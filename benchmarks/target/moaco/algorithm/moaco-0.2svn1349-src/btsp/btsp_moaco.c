...
{
    return powx (value1, 1. - lambda) * powx (value2, lambda);
}
                        ...
                        double alpha, double beta)
{
    ...
    total[i][k] = powx (ph[i][k], alpha) * powx (heu[i][k], beta);
    total[k][i] = total[i][k];
}
                        ...
                        double lambda)
{
    ...
    case WEIGHTED_PRODUCT_AGGREGATION:
        total[i][k] = powx (ph[i][k], alpha)
            * powx (weighted_product_aggregation (lambda, 
    ...
    case WEIGHTED_SUM_AGGREGATION:
        total[i][k] = powx (ph[i][k], alpha)
            * powx (weighted_sum_aggregation (lambda, 
                                             heuristic_info[0][i][k],
                                             heuristic_info[1][i][k]),
                   beta);
        break;
                        ...
                        double lambda)
{
    ...
    case WEIGHTED_SUM_AGGREGATION:
        total[i][k] = 
            powx (weighted_sum_aggregation (lambda, ph1[i][k], ph2[i][k]),
                 alpha)
            * powx (heu[i][k], beta);
        break;
    default: 
                        ...
                        double lambda)
{
    ...
    switch (Pheromone_aggregation_mode) {
    case WEIGHTED_PRODUCT_AGGREGATION:
        total_ik = powx (weighted_product_aggregation (lambda, ph1_ik, ph2_ik),
                        alpha);
        break;
        
    case WEIGHTED_SUM_AGGREGATION:
        total_ik = powx (weighted_sum_aggregation (lambda, ph1_ik, ph2_ik),
                        alpha);
        break;
    default: 
        fprintf (stderr, "%s(): ", __FUNCTION__);
    ...
    switch (Heuristic_aggregation_mode) {
    case WEIGHTED_PRODUCT_AGGREGATION:
        total_ik *= powx (weighted_product_aggregation (lambda, heu1_ik, heu2_ik),
                         beta);
        break;
        
    case WEIGHTED_SUM_AGGREGATION:
        total_ik *= powx (weighted_sum_aggregation (lambda, heu1_ik, heu2_ik),
                         beta);
        break;
    default: 
        fprintf (stderr, "%s(): ", __FUNCTION__);
                               ...
                               double alpha, double beta, double lambda __unused)
{
    if (rph[i][k] < rph[k][i])
        /* force pheromone trails to be symmetric as much as possible */
        rph[k][i] = rph[i][k];
    total[i][k] = powx (rph[i][k], alpha)
        * powx (heuristic_info[0][i][k] + heuristic_info[1][i][k], beta);
    total[k][i] = total[i][k];
}
